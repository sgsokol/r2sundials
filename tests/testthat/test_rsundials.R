...
pfnd=cppXPtr(code='
int d_robertson(double t, const vec &y, vec &ydot, RObject &param) {
...
pfnj=cppXPtr(code='
int jac_robertson(double t, const vec &y, vec &ydot, mat &J, RObject &param) {
  NumericVector p(param);
  J.at(0, 0) = -p["k1"];
  J.at(1, 0) = p["k1"]; 
  J.at(2, 0) = 0.;
...
pfnspj=cppXPtr(code='
int spjac_robertson(double t, const vec &y, vec &ydot, uvec &ir, uvec &pj, vec &v, int n, int nz, RObject &param) {
  if (nz < 7)
...
# sparse Jacobian
#print (system.time(
#out1 <- rsundials::cvode(yini, times, pfnd, param=parms, fjac=pfnspj, nz=8)
#))
# dense Jacobian
#print (system.time(
#out2 <- rsundials::cvode(yini, times, pfnd, param=parms, fjac=pfnj)
#))
...
# pointer to rhs function
pball=cppXPtr(code='
int d_ball(double t, const vec &y, vec &ydot, RObject &param) {
  NumericVector p(param);
  ydot[0] = y[2];
  ydot[1] = y[3];
...
pevt=cppXPtr(code='
int event_ball(double t, const vec &y, vec &ynew, ivec &rootsfound, RObject &param) {
  NumericVector p(param);
...
#outb <- rsundials::cvode(yinib, timesb, pball, paramb, nroot=1, froot=proot, fevent=pevt)
#class(outb)=class(out); plot(outb)

rhs_ball_r=function(t, y, p) {
...
root_ball_r=function(t, y, p) y[2]
event_ball_r=function(t, y, rootsfound, p) {
  nbounce=attr(event_ball_r, "nbounce") # workaround for static variable
  if (is.null(nbounce))
    nbounce=0
  ...
  nbounce=nbounce + 1
  attr(event_ball_r, "nbounce") <<- nbounce;
  ynew=y;
  if (nbounce < p["nbounce"]) {
    # here nbounce=1:4
    ...
    # here nbounce=5
    attr(event_ball_r, "nbounce") <<- 0 # reinit counter for possible next calls to cvode
    return(list(flag=RSUNDIALS_EVENT_STOP, ynew=ynew))
  }
}
#system.time(outbr <- rsundials::cvode(yinib, timesb, rhs_ball_r, paramb, nroot=1, froot=root_ball_r, fevent=event_ball_r))
